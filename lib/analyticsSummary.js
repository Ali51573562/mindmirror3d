export function summarizeAnalytics(events) {
  ...
  const attention={},sections={},symbols={},pages={},buttons={},video={},cards={},segments={},depth={};
  ...
  for(const rows of visits.values()){
    ...
    const clicked=rows.some(e=>['start_journey_click','hero_see_my_free_preview_click','hero_free_preview_click'].includes(e.event_name));
    ...
    const unique={attention:new Set(),sections:new Set(),symbols:new Set(),pages:new Set(),buttons:new Set(),video:new Set(),cards:new Set()};let furthest=0;
    for(const e of rows){const c=e.context;
      if(['homepage_3sec','homepage_5sec','homepage_10sec','homepage_20sec','homepage_30sec','homepage_first_interaction','homepage_scroll_start','hero_image_visible','hero_see_my_free_preview_click'].includes(e.event_name))unique.attention.add(e.event_name);
      if(['hero_free_preview_click','hero_buy_as_gift_click'].includes(e.event_name))unique.attention.add(e.event_name);
      if(['hero_see_my_free_preview_click','hero_free_preview_click'].includes(e.event_name))unique.buttons.add('hero');
    ...
    for(const [key,obj] of Object.entries({attention,sections,symbols,pages,buttons,video,cards}))for(const value of unique[key])if(value!==undefined)increment(obj,value);
  ...
  return {totals,attention,sections,symbols,pages,buttons,video,cards,segments,depth};
}
