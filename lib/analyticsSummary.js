export function summarizeAnalytics(events) {
  ...
  for(const rows of visits.values()){
    ...
    const clicked=rows.some(e=>['start_journey_click','hero_see_my_free_preview_click'].includes(e.event_name));
    ...
    for(const e of rows){const c=e.context;
      ...
      if(e.event_name==='hero_see_my_free_preview_click')unique.buttons.add('hero');
