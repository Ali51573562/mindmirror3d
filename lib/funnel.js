...
function context() {
  ...
  if(new URLSearchParams(location.search).get('analytics_test')==='1')write('mm_internal','1');
  if(new URLSearchParams(location.search).get('analytics_test')==='0')write('mm_internal','0');
  return {browser_id:browser,visit_id:visit.id,version:VERSION,device:innerWidth<768?'mobile':innerWidth<1024?'tablet':'desktop',internal:process.env.NODE_ENV!=='production'||read('mm_internal')==='1',campaign:visit.campaign};
}
...
export async function trackFunnelEvent(eventName,pagePath,userId=null,properties={},once=null) {
  ...
  try {
    ...
    if(!['/','/profile','/auth'].includes(pagePath)){
      const {error}=await supabase.from('funnel_events').insert({session_id:ctx.browser_id,user_id:userId,event_name:eventName,page_path:pagePath});
    ...
    const entry={id:uuid(),session_id:ctx.browser_id,event_name:eventName,page_path:pagePath,created_at:new Date().toISOString(),context:{...ctx,...properties}};
