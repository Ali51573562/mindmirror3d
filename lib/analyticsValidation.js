...
names.add('hero_free_preview_click');
names.add('hero_buy_as_gift_click');
...
export function cleanEvent(e) {
  const loveRoutes = {
    '/love': ['love_view_landing','love_3sec','love_5sec','love_10sec','love_20sec','love_30sec','love_first_interaction','love_scroll_start','love_scroll_50','love_how_it_works_view','love_hero_cta_click'],
    '/love/early-access': ['love_email_form_view','love_email_submit','love_email_submit_success'],
  };
  const loveEvent = loveRoutes[e?.page_path]?.includes(e?.event_name);
  if(!e || !uuid.test(e.id) || !uuid.test(e.session_id) || !(loveEvent || names.has(e.event_name)) || !['/','/profile','/auth',...Object.keys(loveRoutes)].includes(e.page_path))throw Error('Invalid event');
  if(!loveEvent && e.page_path!=='/' && e.event_name!==({'/profile':'view_profile','/auth':'view_auth'}[e.page_path]))throw Error('Invalid route');
  const c=e.context;
  ...
  const enums={section:['hero','how','example','explore','guidebook','video','final'],location:['hero','how','guidebook','final'],symbol:['Cloud head','Heart body','Puzzle hands','Butterfly wings'],action:['turn','enlarge','zoom_turn','text_open'],mode:['inline','enlarged'],direction:['next','previous']};
