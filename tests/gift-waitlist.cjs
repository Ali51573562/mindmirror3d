...
(async()=>{
 ...
 assert.equal(writes[0].row.email,'person@example.com');assert.equal(writes[0].table,'gift_waitlist');assert.equal(writes[0].options.ignoreDuplicates,true);
 assert.equal((await call({email:'person@example.com'})).code,200);
 databaseError={code:'42P01'};assert.equal((await call({email:'person@example.com'})).code,503);
 console.log('Gift waitlist validation, origin checks, normalization, duplicate handling and failure response passed.');
